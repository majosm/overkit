...
collect CreateCollectNoneCol(std::shared_ptr<context> &&Context, comm_view Comm, const cart &Cart,
  ...
  case data_type::BOOL:
    ...
    break;
  case data_type::BYTE:
    Collect = collect_none_col<byte>(std::move(Context), Comm, Cart, LocalRange, CollectMap, Count,
      FieldValuesRange);
    break;
  case data_type::INT:
...
collect CreateCollectAnyCol(std::shared_ptr<context> &&Context, comm_view Comm, const cart &Cart,
  ...
  case data_type::BOOL:
    ...
    break;
  case data_type::BYTE:
    Collect = collect_any_col<byte>(std::move(Context), Comm, Cart, LocalRange, CollectMap, Count,
      FieldValuesRange);
    break;
  case data_type::INT:
...
collect CreateCollectNotAllCol(std::shared_ptr<context> &&Context, comm_view Comm, const cart &Cart,
  ...
  case data_type::BOOL:
    ...
    break;
  case data_type::BYTE:
    Collect = collect_not_all_col<byte>(std::move(Context), Comm, Cart, LocalRange, CollectMap,
      Count, FieldValuesRange);
    break;
  case data_type::INT:
...
collect CreateCollectAllCol(std::shared_ptr<context> &&Context, comm_view Comm, const cart &Cart,
  ...
  case data_type::BOOL:
    ...
    break;
  case data_type::BYTE:
    Collect = collect_all_col<byte>(std::move(Context), Comm, Cart, LocalRange, CollectMap, Count,
      FieldValuesRange);
    break;
  case data_type::INT:
...
collect CreateCollectMinCol(std::shared_ptr<context> &&Context, comm_view Comm, const cart &Cart,
  ...
  case data_type::BOOL:
    ...
    break;
  case data_type::BYTE:
    Collect = collect_min_col<byte>(std::move(Context), Comm, Cart, LocalRange, CollectMap, Count,
      FieldValuesRange);
    break;
  case data_type::INT:
...
collect CreateCollectMaxCol(std::shared_ptr<context> &&Context, comm_view Comm, const cart &Cart,
  ...
  case data_type::BOOL:
    ...
    break;
  case data_type::BYTE:
    Collect = collect_max_col<byte>(std::move(Context), Comm, Cart, LocalRange, CollectMap, Count,
      FieldValuesRange);
    break;
  case data_type::INT:
...
template <typename T> using collect_interp_threaded_col = collect_interp_threaded<T,
...
collect CreateCollectInterpThreadedCol(std::shared_ptr<context> &&Context, comm_view Comm, const
  ...
  switch (ValueType) {
  case data_type::FLOAT:
    Collect = collect_interp_threaded_col<float>(std::move(Context), Comm, Cart, LocalRange,
      CollectMap, Count, FieldValuesRange, InterpCoefs);
    break;
  case data_type::DOUBLE:
    Collect = collect_interp_threaded_col<double>(std::move(Context), Comm, Cart, LocalRange,
      CollectMap, Count, FieldValuesRange, InterpCoefs);
    break;
  default:
    OVK_DEBUG_ASSERT(false, "Invalid data type for interpolation collect operation.");
