...
void Assemble(domain &Domain, const assembly_options &Options) {
  ...
  Domain.Logger_->LogStatus(IsDomainRoot, 0, "Beginning assembly on domain %s.",
    Domain.Name_);
...
//   }
  ...
  if (HasConnectivity) {
    domain_internal::UpdateConnectivitySourceDestRanks(Domain);
  }

  if (HasExchange) {
    domain_internal::UpdateExchanges(Domain);
  }
  ...
  Domain.Logger_->LogStatus(IsDomainRoot, 0, "Finished assembly on domain %s.",
    Domain.Name_);
